...
openssl req -nodes -new -sha256 -key server.key -subj "/CN=${DOMAIN}" -out server.csr
openssl x509 -req -days 3650 -in server.csr -signkey server.key -out server.crt -extfile <(printf "\nsubjectAltName='DNS:${DOMAIN}")
openssl x509 -in server.crt -out server.der -outform DER

export DOMAIN=localhost
export LOCALHOST=127.0.0.1
echo quit | openssl s_client -showcerts -servername "${DOMAIN}" -connect "${ILB_IP}":8080 > server.pem

curl -X POST \
    --resolve ${DOMAIN}:8080:${LOCALHOST} \
    -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
    -H "Content-Type:application/json" \
    --cacert server.pem https://$DOMAIN:8080/webhook \
    -d '{
        "fulfillmentInfo":{
...
export LOCALHOST=10.10.20.2
export PORT=443
echo quit | openssl s_client -showcerts -servername "${DOMAIN?}" -connect "${ILB_IP?}":${PORT?} > server.pem
curl -X POST \
    --resolve ${DOMAIN}:${PORT?}:${LOCALHOST} \
    -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
    -H "Content-Type:application/json" \
    --cacert server.pem https://$DOMAIN:${PORT?} \
    -d '{
        "fulfillmentInfo":{
        "tag":"validatePhoneLine"
        },
...
'''
# '{"fulfillmentInfo":{"tag":"validatePhoneLine"},"sessionInfo":{"parameters":{"phone_number":"123456"}}'

# result = requests.post(audience, json={"fulfillmentInfo":{"tag":"validatePhoneLine"},"sessionInfo":{"parameters":{"phone_number":"123456"}}}, headers=new_headers)
